...
public class GainAdjuster : MonoBehaviour {
	...
	void Start () {
        ...
        GameMain.EvtBGClearAllData_Before += Handle_BackGroundClearAllDataBefore;//在后台清所有数据0的时候.重置用于计算抽放水的盈亏币数
        ...
        {
            Debug.LogError("Dat_GainAdjustIdx 在有效值范围以外.");
            return;
        }
        ...
        //Debug.Log("mCurLossProfitScore = " + mCurLossProfitScore);
    ...
    void Handle_PlayerFire(Player owner, Gun gun, int useScore)
    {
        //Debug.Log((mIsLossOrProfit ? "放水中" : "抽水中") + "  GainAdjustIdx =" + mBss.Dat_GainAdjustIdx.Val +"    gainOdd = "+GameOdds.GainRatio);
        mScoreGain += useScore;
    }

    void Handle_PlayerGainScoreFromFish(Player p, int scoreGetted, Fish firstFish,Bullet b)
    {
        ...
        while (true)
        {
            ...
            if (mIsLossOrProfit)// 放水
            {
                if ((mScoreLoss - mScoreGain) > mCurLossProfitScore)//(机台亏损 - 机台盈利) 达到放水额
                {
                    ...
                    yield break;
                    //Debug.Log("放水完毕 = " + (mScoreLoss - mScoreGain));
                    //Destroy(gameObject);
                }
            }
            else// 抽水
    ...
    void Handle_BackGroundClearAllDataBefore()
    {
        ...
        //Debug.Log("set gain loss score zero");
    }
